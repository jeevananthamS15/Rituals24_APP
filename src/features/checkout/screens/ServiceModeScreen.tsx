    ...
    {Array.from({ length: totalSteps }).map((_, index) => {
      ...
      return (
        <View
          ...
          style={[
            pb.line,
            isActive ? pb.active : pb.inactive,
          ]}
        />
      );
    })}
...
const ModeCard: React.FC<{
  ...
  const cardBg = selected && isHomeVisit ? PRIMARY : selected ? PRIMARY : SURFACE;
      ...
      {selected && item.setup && (
        <>
          ...
          <View style={card.extraRow}>
           ...
           <Settings size={scale(13)} color={TEXT_ON_DARK} strokeWidth={1.8}/>
               <Text style={card.extraLabel}>Setup:</Text>
              <Text style={card.extraValue}>{item.setup}</Text>
            </View>
        
            <View style={card.extraLine}>
           <BadgeIndianRupee size={scale(13)} color={TEXT_ON_DARK} strokeWidth={1.8}/>
...
export const ServiceModeScreen: React.FC<{ navigation?: any }> = ({ navigation }) => {
  const [selectedMode, setSelectedMode] = useState<PujaMode | null>('home_visit');
  const insets = useSafeAreaInsets();
    ...
    <View style={s.container}>
      ...
      >
        ...
        <Text style={s.subheading}>
          Choose a service mode that works best for you. Each mode is fully sacred and verified.
        </Text>
      ...
      <View style={[s.footer, { paddingBottom: insets.bottom + scale(8) }]}>
        <TouchableOpacity
          ...
          activeOpacity={0.88}
          onPress={() => navigation?.navigate('DateMuhurat')}
